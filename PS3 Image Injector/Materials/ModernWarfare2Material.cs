...
namespace PS3_Image_Injector {
    public class ModernWarfare2Material : IMaterial {
        static TMAPI PS3 = new TMAPI();

        static readonly UInt32 ASSET_IMAGE_COUNT = 1937;
        static readonly UInt32 ASSET_IMAGE_START = 0xC67D2C;
        ...
        public ModernWarfare2Material(UInt32 Pointer) {
        ...
        public static void Populate(MainForm mainForm) {
            ...
            for (uint i = 0; i < ASSET_IMAGE_COUNT; i++) {
                ...
                if (StringName != 0) {
                    IMaterial material = new ModernWarfare2Material(ptr);

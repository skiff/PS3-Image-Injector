...
namespace PS3_Image_Injector {
    public class GhostsMaterial : IMaterial {
        static TMAPI PS3 = new TMAPI();

        static readonly UInt32 ASSET_IMAGE_COUNT = 4079;
        static readonly UInt32 ASSET_IMAGE_START = 0x22ADA4C;
        ...
        public GhostsMaterial(UInt32 Pointer) {
        ...
        public static void Populate(MainForm mainForm) {
            ...
            for (uint i = 0; i < ASSET_IMAGE_COUNT; i++) {
                ...
                if (StringName != 0) {
                    IMaterial material = new GhostsMaterial(ptr);
